...
private let mainURL = "https://insight.maxcore.io/api/addrs/utxo"
private let fallbackURL = "https://insight.maxcore.io/api/addrs/utxo"
private let testnetURL = "https://testnet.maxcore.io/api/addrs/utxo"
...
class StartImportViewController : UIViewController {
    ...
    private let header = RadialGradientView(backgroundColor: .purple, offset: 64.0)
    ...
    private func checkBalance(key: BRKey) {
        present(balanceActivity, animated: true, completion: {
            var key = key
            guard let address = key.addressMW() else { return }
